...
import pt.uminho.ceb.biosystems.merlin.local.alignments.core.PairwiseSequenceAlignement;
 ...
 *
 */
public class RunSimilaritySearchBigBlast implements PropertyChangeListener {
...
//	private String tcdbFastaFilePath;
	...
	final static Logger logger = LoggerFactory.getLogger(RunSimilaritySearchBigBlast.class);
	 ...
	 * @throws Exception
	 */
	public RunSimilaritySearchBigBlast(Map<String, AbstractSequence<?>> staticGenesSet, double similarity_threshold, Method method, ConcurrentHashMap<String, AbstractSequence<?>> querySequences, 
...
//		ConcurrentLinkedQueue<String> queryArray = new ConcurrentLinkedQueue<String>(this.querySequences.keySet());
		...
		this.subjectFastaFilePath = this.workspaceTaxonomyFolderPath.concat("/subject.faa");
		...
		CreateGenomeFile.buildFastaFile(this.subjectFastaFilePath , this.staticGenesSet);
	...
	public Pair<ConcurrentLinkedQueue<AlignmentContainer>,ConcurrentLinkedQueue<AlignmentContainer>> runBBBlastHits(//String queryGenomeFilePath, 
			//String subjectGenomeFilePath, 
			boolean isTransportersSearch, Double eValueThreshold, Double bitScoreThreshold, 
			Double queryCoverageThreshold, Double targetCoverageThreshold) throws Exception{
...
//		secondBlastQuerySequences.putAll(FastaReaderHelper.readFastaProteinSequence(subjectGenomeFastaFile));
		...
		ConcurrentHashMap<String, AbstractSequence<?>> secondBlastQuerySequences = new ConcurrentHashMap<>(this.staticGenesSet);
